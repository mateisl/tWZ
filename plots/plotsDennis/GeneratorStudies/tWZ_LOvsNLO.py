...
files = [
    ...
    ("LO + jet (new matching nJetMax=1)", "/users/dennis.schwarz/CMSSW_10_6_20/src/tWZ01j_newMatching/tWZ01j-vec_newMatching_50000events.root", ROOT.kAzure+7),
...
for (name,filename,color) in files:
    ...
    for event in tree:
        ...
        for i in range(event.nGenJet):
            h_etajets[name].Fill(event.GenJet_eta[i])
            h_ptjets[name].Fill(event.GenJet_pt[i])
            if event.GenJet_pt[i] > 30 and abs(event.GenJet_eta[i]) < 2.4:
                Njets30 += 1
                if event.GenJet_partonFlavour[i] == 5:
                    Nbjets +=1
            if event.GenJet_pt[i] > 40 and abs(event.GenJet_eta[i]) < 2.4:
                Njets40 += 1
            if event.GenJet_pt[i] > 50 and abs(event.GenJet_eta[i]) < 2.4:
                Njets50 += 1
