...
if args.era == "Run2016":
    if args.partonweight: 
        ...
        mc = [Summer16.TWZ, Summer16.TTZ, Summer16.TTX_rare, Summer16.TZQ, Summer16.WZ, Summer16.triBoson, Summer16.ZZ, Summer16.nonprompt_3l]#Summer16.yt_tWZ01j_filter
...
def drawPlots(plots, mode, dataMCScale):
  for log in [False, True]:
    plot_directory_ = os.path.join(plot_directory, 'analysisPlots', 'controlRegion', args.plot_directory, args.era, mode + ("_log" if log else ""), args.selection)
    for plot in plots:
...
def getMWZ( event, sample ):
    # get the invariant mass of the 3l+misspt system
    l = []
    for i in range(3):
        l.append(ROOT.TLorentzVector())
        l[i].SetPtEtaPhiM(event.lep_pt[i], event.lep_eta[i], event.lep_phi[i],0)
    l.append(ROOT.TLorentzVector())
    l[3].SetPtEtaPhiM(event.met_pt, 0, event.met_phi,0)
    event.MWZ = (l[0] + l[1] + l[2]+ l[3]).M()
...
#invariantMass WZ
    plots.append(Plot(
        name = "M_WZ",
        texX = 'M(WZ) (GeV)', texY = 'Number of Events',
